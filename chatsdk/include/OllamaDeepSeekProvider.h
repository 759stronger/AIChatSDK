...
namespace chatsdk
{
    class OllamaDeepSeekProvider : public LLMProvider{  
    public:
        // OllamaDeepSeekProvider() = default;
        // ~OllamaDeepSeekProvider() = default;
        ...
        // 发送消息 全量返回
        std::string sendMessage(const std::vector<Message>& messages, const std::map<std::string , std::string>& request_param) override;
        // 发送消息 流式返回
     ...
     protected:
        std::string _model_name_; // 模型名称
        std::string _model_desc_; // 模型描述信息1
    };
}